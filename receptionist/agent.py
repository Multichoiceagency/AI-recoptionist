...
class Receptionist(Agent):
    ...
    async def on_enter(self) -> None:
        await self.session.generate_reply(
            instructions=(
                f"Begroet de beller met: '{self.config.greeting}' "
                f"Spreek altijd Nederlands. Stel daarna direct een open vraag om te begrijpen "
                f"waarmee je de beller kunt helpen. Wacht niet tot de beller alles uitlegt — "
                f"jij neemt het initiatief."
            )
        )
